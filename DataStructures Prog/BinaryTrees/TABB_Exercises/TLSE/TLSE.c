...
TLSE *pushSorted(TLSE *L, int elem) // Only works with sorted lists
{
    TLSE *aux_old = NULL;
    TLSE *aux_actual = L;
    while(aux_actual && aux_actual->lim_inferior < elem)
    {
        aux_old = aux_actual;
...
{
    TLSE *aux = l;
    while((aux) && (aux->lim_inferior != elem)) // while aux != NULL and ...
        aux = aux->next;
    return aux;
}
TLSE* search_rec(TLSE *l, int elem)
{
    if((!l) || (l->lim_inferior == elem)) // while l == NULL or l == node wanted, return
        return l;
...
TLSE* pop(TLSE *l, int elem)
{
    TLSE *actual = l;
    TLSE *old = NULL;
    while((actual) && (actual->lim_inferior != elem))
    {
        old = actual;
        actual = actual->next;
    }
...
void print_inv(TLSE *L)
{
    ...
    while(aux)
    {
        L_aux = pushStart(L_aux, aux->lim_inferior);
        aux = aux->next;
    }
    TLSE_print(L_aux);
    freeAll(L_aux);
